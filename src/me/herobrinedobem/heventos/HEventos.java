 ...
 * funcoes serao chamadas! Refeito por Espc7 para Covil Server!
 * 
...
public class HEventos extends JavaPlugin {
	...
	public void onEnable() {
		Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fIniciando o plugina!");
		loadConfigs();
		loadDatabase();
		loadCommands();
		loadListeners();
		...
		Bukkit.getConsoleSender().sendMessage(
				"§9[CovilHEventos] §fPlugin Habilitado - (Versao §9" + this.getDescription().getVersion() + "§f)");
		Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fRefeito por §aEspc7 (Covil)");
	}
	...
	public void onDisable() {
		...
		Bukkit.getConsoleSender().sendMessage(
				"§9[CovilHEventos] §fPlugin Desabilitado - (Versao §9" + this.getDescription().getVersion() + "§f)");
	}
	...
	private void loadConfigs() {
		if (!new File(this.getDataFolder(), "config.yml").exists()) {
	        ...
	        saveConfig();
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fConfig.yml criada com sucesso!");
		} else {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fConfig.yml carregada com sucesso!");
		}
		File eventosFile = new File(this.getDataFolder() + File.separator + "Eventos");
		if (!eventosFile.exists()) {
			eventosFile.mkdirs();
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fPasta 'Eventos' criada com sucesso!");
		}
		if (this.getConfig().getBoolean("Ativar_Configs_Exemplos")) {
			...
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fConfigs de exemplos criadas!");
		}
	...
	private void loadDependencies() {
		if (!setupSimpleClans()) {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §cSimpleClans n§o encontrado!");
		}
		if (!setupEconomy()) {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §cVault n§o encontrado!");
		}
		if (!setupLegendChat()) {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §cLegendChat n§o encontrado!");
		}
	}
	...
	private void loadDatabase() {
		if (this.getConfig().getBoolean("MySQL.Ativado") == true) {
			...
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fMySQL Habilitado!");
		} else {
			File databaseFile;
			...
			if (!databaseFile.exists()) {
				...
				Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fDatabase.db criada com sucesso!");
			}
			databaseManager = new Database(DatabaseType.SQLITE);
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fSQLite Habilitado!");
		}
	}
	...
	private boolean setupEconomy() {
		...
		Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fVault encontrado com sucesso!");
	...
	private boolean setupSimpleClans() {
		...
		if (plug != null) {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fSimpleClans encontrado com sucesso!");
	...
	private boolean setupLegendChat() {
		...
		if (plug != null) {
			Bukkit.getConsoleSender().sendMessage("§9[CovilHEventos] §fLegendChat encontrado com sucesso!");
	...
	public static HEventos getHEventos() {
		return (HEventos) Bukkit.getServer().getPluginManager().getPlugin("CovilHEventos");
	}
}
